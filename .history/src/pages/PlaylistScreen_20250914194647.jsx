...
export default function PlaylistScreen() {
 const allSongs = useMemo(() => RosesOfRomeSongs(), []);
  ...
  useEffect(() => {
    if (selectedAlbum === "all") {
      ...
      if (favorites.length > 0) {
        setFilteredSongs(
          allSongs.filter((song) => favorites.includes(song.songFile))
        );
      }
      else {
        setSelectedAlbum("all"); // default
  ...
  }, [selectedAlbum, allSongs,navigate]);
    ...
    <>
      {selectedAlbum && (
        <div className="album-description">
          ...
          <div className="controls-button"></div>
          ...
          {selectedAlbum === "Splendore" && (
            <button
              className="share-button red"
              onClick={() => {
                // Clear the favorites array in localStorage
                // localStorage.removeItem("favorites");
                // OR if you want to reset it as an empty array instead:
                 localStorage.setItem("favorites", JSON.stringify([]));

                // Optional: trigger state update so UI refreshes
