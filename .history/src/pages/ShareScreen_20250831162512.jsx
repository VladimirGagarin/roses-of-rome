...
export default function ShareScreen() {
  ...
  const [isFromMeritoAlbum, setIsFromMeritoAlbum] = useState(false);
  const [isShowingShareOverlay, setIsShowingShareOverlay] = useState(false);
  ...
  useEffect(() => {
    ...
    const foundSong = allSongs.find((song) => {
      // Trim and normalize both IDs
      const normalizedSongId = String(song.songId).trim().toLowerCase();
      const normalizedParamId = String(id).trim().toLowerCase();
      return normalizedSongId === normalizedParamId;
    });

    if (foundSong) {
      setCurrentSongObj(foundSong);
      if (foundSong.songAlbum && foundSong.songAlbum === "Merito") {
        setIsFromMeritoAlbum(true);
      }
    } else {
      ...
      const  handleYoutube = (song) => {
        if(!song) return;

        const youtubeUrl = song.songLink || `https://www.youtube.com/results?search_query=${encodeURIComponent(song.songName[language] || song.songName.en)}`;
  ...
  const  handleShare = () => {
    ...
    const shareUrl = `${window.location.origin}/roses-of-rome/#/pages/share/${currentSongObj.songId}?utm_source=app&utm_medium=share&utm_campaign=song_share`;

    const shareData = {
      title: currentSongObj.songName[language] || currentSongObj.songName.en,
      text:
        language === "it"
          ? `Ascolta "${currentSongObj.songName[language]}" 🎶`
          : `Listen to "${currentSongObj.songName[language]}" 🎶`,
      url: shareUrl,
    };
          ...
          <div className="more-action-card">
            ...
            {isFromMeritoAlbum && (
              <button
                ...
                onClick={() => handleYoutube(currentSongObj)}
                title={language === "it" ? "Guarda su Youtube" : "View on Youtube"}
                aria-label={
                  language === "it" ? "Guarda su Youtube" : "View on Youtube"
                }
              >
                <FaYoutube />
            ...
            {Array.isArray(currentSongObj.songLyrics) &&
              currentSongObj.songLyrics.length > 0 && !isFromMeritoAlbum && (
                <button
                  className="surprise-button"
