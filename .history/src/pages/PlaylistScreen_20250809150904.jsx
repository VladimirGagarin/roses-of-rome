...
import { FaYoutube, FaShareAl } from "react-icons/fa";
...
export default function PlaylistScreen() {
  ...
  const handleShareSong = async (song) => {
    // Generate song-specific URL (e.g., "https://yourdomain.com/pages/share/rorMusic_002")
    const songUrl = `${window.location.origin}/pages/share/${song.songId}`;
    const songTitle = song.songName[language] || song.songName.en;
    const shareText =
      language === "it"
        ? `Ascolta "${songTitle}" dall'album ${song.songAlbum} 🎶`
        : `Listen to "${songTitle}" from ${song.songAlbum} 🎶`;
    ...
    <div className="playlist-container">
      {allSongs.map((song) => (
        <div className="song-card" key={song.songId}>
          ...
          {Array.isArray(song.songLyrics) && song.songLyrics.length > 0 && audioState !== "waiting" && (
