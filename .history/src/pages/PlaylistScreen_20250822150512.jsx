...
const albumArray = (array) => {
  const newArr = array.map((s) => s.songAlbum);

  const list = newArr.reduce((albums, albumName) => {
    if (!albums.includes(albumName)) {
      albums.push(albumName);
    }
    return albums; // Important: return the accumulator
  }, []);
...
export default function PlaylistScreen() {
  ...
  useEffect(() => {
    if (selectedAlbum === 'all') {
      ...
      setFilteredSongs(allSongs.filter(song => song.songAlbum === selectedAlbum));
    }
