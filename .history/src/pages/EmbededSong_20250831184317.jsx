...
export default function EmbedSong() {
  ...
  useEffect(() => {
    ...
    if (audio && song && song.songLyrics && song.songLyrics.length > 0) {
      const handleTimeUpdate = () => {
        const currentTime = audio.currentTime * 1000; // Convert to milliseconds
        const currentLyricObj = song.songLyrics.find(
          (lyric) => currentTime >= lyric.start && currentTime <= lyric.end
        );

        const currentLyric = currentLyricObj?.text?.[language] || "🎶";

        setCurrentLine(currentLyric);
  ...
  return (
    <div className="embed-container">
      <audio src={song.songFile} ref={audioRef}></audio>
      ...
      />
      {/*lyrics field */}
      <div className="lyrics-container-embed">
        {currentLine && <p className="current-line-embed">{currentLine}</p>}
      </div>
