...
import { FaPlay, FaPause, FaRetweet, FaSyncAlt, FaSpinner, FaRegHeart, FaHeart } from "react-icons/fa";
...
export default function AudioComponent({ audioFile, title }) {
  ...
  useEffect(() => {
    const savedLikeStatus = localStorage.getItem(`audio-like-${audioFile}`);
  ...
  const handleLike = () => {
    setIsLiked((prev) => {
      ...
      localStorage.setItem(`audio-like-${audioFile}`, newLikeStatus);
  ...
  return (
    <div className={`audio-component ${isCurrent ? "current" : ""}`} ref={audioComponent}>
      {!isOnline ? (
